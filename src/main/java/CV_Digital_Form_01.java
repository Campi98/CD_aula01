...
public class CV_Digital_Form_01 extends javax.swing.JFrame {
    ...
    private void addPessoaButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addPessoaButtonActionPerformed
        ...
        // Nome do ficheiro baseado no email
        File pessoaFile = new File(email + ".txt");

        if (pessoaFile.exists()) {
            JOptionPane.showMessageDialog(this, "O email já existe no sistema!");
        } else {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(pessoaFile))) {
                writer.write("Nome: " + nome);
                writer.newLine();
                writer.write("Email: " + email);
                writer.newLine();
                writer.write("Eventos: " + eventos);
                writer.newLine();
        ...
        if (listOfFiles != null) {
            for (File file : listOfFiles) {
                try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
                    String firstLine = reader.readLine();  // A primeira linha contém o nome
                    String nome = firstLine.replace("Nome: ", "");
                    pessoasList.append(nome).append("\n");
                } catch (IOException ex) {
                    ex.printStackTrace();
                }
            }
    ...
    private void verCVButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_verCVButtonActionPerformed
        ...
        if (pessoaFile.exists()) {
            try (BufferedReader reader = new BufferedReader(new FileReader(pessoaFile))) {
                StringBuilder cv = new StringBuilder();
                String line;
                while ((line = reader.readLine()) != null) {
                    cv.append(line).append("\n");
                }
